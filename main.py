...
# Тимчасові структури для анонімного чату
WAITING_ROOM = []
ACTIVE_CHATS = {}
...
# --- ПЕРЕГЛЯД ВЛАСНОЇ АНКЕТИ ---
@dp.message(F.text == "👤 Моя анкета")
async def my_profile(message: types.Message):
    user_id = message.from_user.id
    conn = get_db()
    user = conn.execute("SELECT name, age, location, bio, photo FROM users WHERE user_id = ?", (user_id,)).fetchone()
    conn.close()
    
    if user:
        name, age, location, bio, photo = user
        caption = f"👤 **Твоя анкета:**\n\n🏷 **Ім'я:** {name}, {age}\n📍 **Локація:** {location}\n📝 **Про себе:** {bio}"
        await bot.send_photo(user_id, photo=photo, caption=caption, parse_mode="Markdown")
    else:
        await message.answer("Ти ще не створив анкету. Напиши /start")

# --- ПЕРЕГЛЯД ІНШИХ АНКЕТ ---
@dp.message(F.text == "❤️ Дивитись анкети")
async def view_profiles(message: types.Message):
    user_id = message.from_user.id
    conn = get_db()
    target = conn.execute("SELECT user_id, name, age, location, bio, photo FROM users WHERE user_id != ? ORDER BY RANDOM() LIMIT 1", (user_id,)).fetchone()
    conn.close()
    
    if not target:
        return await message.answer("🌍 Поки що немає інших анкет у системі. Запроси друзів, щоб почати гортати!")
        
    t_id, t_name, t_age, t_location, t_bio, t_photo = target
    caption = f"🔥 **Знайдено когось новенького!**\n\n🏷 **Ім'я:** {t_name}, {t_age}\n📍 **Локація:** {t_location}\n📝 **Про себе:** {t_bio}"
    await bot.send_photo(user_id, photo=t_photo, caption=caption, reply_markup=keyboards.get_profile_action_keyboard(t_id), parse_mode="Markdown")
...
# --- ВЕРИФІКАЦІЯ ТА ТОП-5 (ЗАГЛУШКИ, ЩОБ КНОПКИ НЕ ВИСЛИ) ---
@dp.message(F.text == "☑️ Пройти верифікацію")
async def req_verify(message: types.Message):
    await message.answer("📸 Запит надіслано! Шеф перевірить твою анкету. Очікуй синю галочку найближчим часом.")
    try:
        await bot.send_message(ADMIN_ID, f"🔔 **ЗАПИТ НА ВЕРИФІКАЦІЮ!**\nКористувач: @{message.from_user.username} (ID: `{message.from_user.id}`)")
    except:
        pass

@dp.message(F.text == "🏆 ТОП-5 Борислава")
async def top_profiles(message: types.Message):
    await message.answer("🏆 **ТОП-5 активних анкет міста:**\n\n1. Анкети оновлюються щотижня за кількістю отриманих лайків! Поки що збираємо статистику. Будь першим!")

@dp.message(F.text == "⚡ Побачення наосліп")
async def speed_date(message: types.Message):
    await message.answer("⚡ **Побачення наосліп**\nРежим запускається щоп'ятниці о 20:00! Бот автоматично з'єднує хлопців та дівчат для швидкого 5-хвилинного спілкування без фото.")
