import logging, asyncio, random, aiosqlite
...
# Ті самі атмосферні ідеї під Борислав!
BORYSLAV_IDEAS = [
...
class Registration(StatesGroup):
    name = State(); age = State(); gender = State(); location = State(); bio = State(); photo = State()
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, name TEXT, age INTEGER, gender TEXT, location TEXT, bio TEXT, photo TEXT, is_verified INTEGER DEFAULT 0, is_banned INTEGER DEFAULT 0, is_active INTEGER DEFAULT 1, in_top INTEGER DEFAULT 1)")
        await db.execute("CREATE TABLE IF NOT EXISTS ads (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, name TEXT, text TEXT)")
        await db.commit()
...
# --- ПОВНОЦІННИЙ ФУНКЦІОНАЛ АДМІНІСТРАТОРА ---
@dp.message(F.text == "🛠 Адмін-панель")
async def admin_btn(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    await state.clear()
    await message.answer("🛠 **Панель адміністратора активована!**\nОбери дію на клавіатурі нижче:", reply_markup=get_admin_menu())
...
async def admin_stats(message: types.Message):
    ...
    stats_text = f"📊 **СТАТИСТИКА БОТА**\n\n👥 Всього людей: {total}\n🟢 Активні в пошуку: {active}\n🔴 Забанені: {banned}"
    await message.answer(stats_text)

@dp.message(F.text == "📢 Зробити розсилку")
async def broadcast_start(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    await message.answer("✍️ Надішли мені текст розсилки для користувачів:")
...
async def ban_finish(message: types.Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID: return
    if not message.text.isdigit(): return await message.answer("❌ ID має бути цифровим!")
    target = int(message.text)
    await state.clear()
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET is_banned=1, is_active=0 WHERE user_id=?", (target,))
        await db.commit()
    await message.answer(f"🔴 Користувача `{target}` успішно забанено!")
...
async def unban_finish(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await message.answer(f"🟢 Користувача `{target}` розбанено!")
...
async def search_finish(message: types.Message, state: FSMContext):
    ...
    if not user: return await message.answer("❌ Анкету не знайдено.")
    b_status = "🔴 В БАНІ" if user['is_banned'] else "🟢 Активний"
    caption = f"🔍 **Знайдено користувача:**\n👤 {user['name']}, {user['age']} років\n📍 {user['location']}\n🔗 @{user['username']}\n\n💬 {user['bio']}\n\nСтатус: {b_status}"
    await bot.send_photo(ADMIN_ID, photo=user['photo'], caption=caption)
  # --- АНОНІМНА РУЛЕТКА (Чат через бота) ---
...
async def roulette_start(message: types.Message):
    ...
    if len(ROULETTE_QUEUE) >= 2:
        ...
        await bot.send_message(p1, "🎉 **Співрозмовника знайдено!** Спілкуйтеся прямо тут. Для виходу тисни кнопку нижче.", reply_markup=get_roulette_menu())
        await bot.send_message(p2, "🎉 **Співрозмовника знайдено!** Спілкуйтеся прямо тут. Для виходу тисни кнопку нижче.", reply_markup=get_roulette_menu())
    else:
...
async def roulette_exit(message: types.Message):
    ...
    if uid in ROULETTE_QUEUE:
        ...
        try: 
            await bot.send_message(partner, "🚫 Співрозмовник завершив діалог.", reply_markup=get_main_menu(partner))
        except: pass
...
async def roulette_forward(message: types.Message):
    ...
    try:
        if message.text: 
            ...
            await bot.send_photo(partner, photo=message.photo[-1].file_id, caption="📸 Фото")
    except: pass
...
async def reg_photo(message: types.Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR REPLACE INTO users (user_id, username, name, age, gender, location, bio, photo) VALUES (?,?,?,?,?,?,?,?)", (message.from_user.id, message.from_user.username, data['name'], data['age'], data['gender'], data['loc'], data['bio'], message.photo[-1].file_id))
        await db.commit()
...
async def my_profile(message: types.Message):
    ...
    # НОВИЙ СУЧАСНИЙ ДИЗАЙН (Як у скріншоті 2, але чистий і мінімалістичний)
