...
GUESTURES = ["👌 (знак ОК)", "👍 (палець вгору)", "🙌🏻 (підняті догори долоні)"]
...
# Мідлварь-функція без декоратора (викликається всередині хендлерів)
        ...
        if u['is_under_check'] == 1:
            if state:
                ...
                if current_state == ManualCheckState.wait_photo.state:
                    return True
            await message.answer("⚠️ Твій профіль відправлено на перевірку адміном!")
            return True
    return False
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if user:
        ...
        if user['is_under_check'] == 1:
            gesture = random.choice(GUESTURES)
            await state.update_data(required_gesture=gesture)
            await message.answer(f"⚠️ Твій профіль під перевіркою адміна.\n\n📸 Будь ласка, сфотографуйся і покажи рукою цей жест: **{gesture}** і надішли фото сюди:")
...
# --- ХЕНДЛЕР ОЧІКУВАННЯ ФОТО ПЕРЕВІРКИ (ПЕРЕНЕСЕНО ВИЩЕ) ---
...
async def manual_check_photo_received(message: types.Message, state: FSMContext):
    ...
    gesture = state_data.get("required_gesture", "Не визначено (перевір візуально)")
    
    await message.answer("⏳ Фото успішно надіслано адміну. Очікуй рішення, функції меню заблоковані.")
    await state.clear()
    
    uid = message.from_user.id
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Підтвердити особистість", callback_data=f"man_confirm_{uid}"),
    ...
    await bot.send_photo(
        ADMIN_ID, 
        photo=photo_id, 
        caption=f"📸 **ФОТО З ПЕРЕВІРКИ НА ФЕЙК**\n🆔 ID: `{uid}`\n👤 Юзер: @{message.from_user.username or 'немає'}\n\n💡 **Людина мала зобразити жест:**\n👉 {gesture}", 
        reply_markup=kb
    )
...
async def reg_bio(message: types.Message, state: FSMContext):
    ...
    await message.answer("📸 Надішли одне гарне photo для профілю:")
...
# --- МІНІМАЛІСТИЧНИЙ ПРОФІЛЬ (БЕЗ РЯДКА "КОГО ШУКАЄ") ---
...
async def date_idea_cmd(message: types.Message):
    ...
    await message.answer(f"💡 **Нова романтична ідея для тебе:**\n\n{idea}")
...
async def book_back_nav(call: CallbackQuery):
    page = int(call.data.split("_")[2])
...
async def book_view_profile(call: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="↩️ Назад до книги", callback_data=f"book_back_{page}")]
    ])
    caption = (
        f"📋 **АНКЕТА З КНИГИ БАЗИ**\n"
        f"🆔 ID: `{user['user_id']}`\n"
...
async def admin_actions_execution(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH, timeout=30) as db:
        if action == "verify":
            ...
            await call.message.answer(f"🔍 Користувачу `{uid}` заблоковано меню. Очікуємо селфі з жестом.")
            
            # Генеруємо жест при виклику з адмінки
            ustate = dp.fsm.get_context(bot=bot, chat_id=uid, user_id=uid)
            gesture = random.choice(GUESTURES)
            await ustate.update_data(required_gesture=gesture)
            await ustate.set_state(ManualCheckState.wait_photo)
            
            try: await bot.send_message(uid, f"⚠️ Адміністрація вимагає пройти обов'язкову перевірку особистості!\n\n📸 Будь ласка, сфотографуйся і покажи рукою цей жест: **{gesture}**.\nНадішли актуальне селфі-фото прямо сюди для розблокування:")
            except: pass
        elif action == "fake":
...
# --- CALLBACK ДЛЯ КНОПОК ПІДТВЕРДЖЕННЯ ФЕЙКІВ ---
...
async def manual_check_decision(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH, timeout=30) as db:
        if decision == "confirm":
            ...
            await call.message.edit_caption(caption="🟢 Перевірка пройдена успішно. Профіль верифіковано.")
            try: await bot.send_message(uid, "🎉 Перевірку пройдено! Твій профіль успішно розблоковано та верифіковано.", reply_markup=get_main_menu())
            except: pass
        elif decision == "fake":
            await db.execute("UPDATE users SET is_under_check = 0, is_verified = 0, fake_status = '(ФЕЙК ❌)' WHERE user_id = ?", (uid,))
            await call.message.edit_caption(caption="❌ Заявку відхилено модератором. Видано плашку ФЕЙК.")
            try: await bot.send_message(uid, "⚠️ Тобі відмовлено у верифікації особистості. Профіль отримав плашку (ФЕЙК).", reply_markup=get_main_menu())
            except: pass
        await db.commit()
    await call.answer()

# --- ІНШІ CALLBACK-ОБРОБНИКИ (ПРОФІЛЬ / ЛАЙКИ) ---
...
async def admin_delete_ad(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH, timeout=30) as db:
        ...
        await db.commit()
    await callback.message.edit_text("❌ *Оговорення видалено.*", parse_mode="Markdown")
...
# --- УНІВЕРСАЛЬНИЙ ОБРОБНИК МУЛЬТИ-ЧАТІВ ТА ТЕКСТУ (В САМОМУ КІНЦІ!) ---
